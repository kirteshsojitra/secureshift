...
app.post('/api/send-schedule', async (req, res) => {
    try {
        ...
        await transporter.sendMail({
            ...
            html: `<div style="font-family:Arial,sans-serif;max-width:720px;margin:0 auto;border:1px solid #e2e8f0;border-radius:10px;overflow:hidden">
        ...
        <div style="padding:20px 24px;background:#f8fafc;border-bottom:1px solid #e2e8f0">
          ...
          <p style="font-size:14px;color:#374151;margin:8px 0 0">Here is your schedule for <strong>${weekLabel}</strong>.</p>
        </div>
        <div>${html}</div>
        <div style="padding:16px 24px;background:#f1f5f9;border-top:1px solid #e2e8f0">
          <p style="font-size:11px;color:#94a3b8;margin:0">Automated message from SecureShift. Do not reply.</p>
        </div>
      </div>`,
        });
...
// ── CLEANUP: Remove duplicate assignments from MongoDB ────────────────────────
app.post('/api/assignments/dedupe', async (req, res) => {
    try {
        ...
        all.forEach(a => {
            ...
            if (seen.has(key)) toDelete.push(a._id);
            else seen.set(key, a._id);
        });
        if (toDelete.length) await Assignment.deleteMany({ _id: { $in: toDelete } });
        res.json({ removed: toDelete.length, remaining: all.length - toDelete.length });
